...
const Map = ({ tourData }) => {
  ...
  useEffect(() => {
    const setUserLocation = async () => {
      ...
      if (status !== "granted") {
        setErrorMsg(
          "Permission to access location was denied. Please enable location data."
        );
        return;
      }
      populateTourMarkers();
  ...
  const getSiteCard = (event) => {
    const siteMarker =
      event.target._internalFiberInstanceHandleDEV.memoizedProps.site;
    setSite(siteMarker);
    setVisible(!visible);
  };
      ...
      <View style={styles.mapContainer}>
        ...
        >
          {tourMarkers.map((site) => {
            const { siteId, siteName, siteDescription, latitude, longitude } =
              site;
            return (
              <Marker
                key={siteId}
